...
counts_18 <- read.table("GSE179718_series_matrix.txt.gz", 
                        header = TRUE, 
                        row.names = 1, 
                        sep = "\t", 
                        comment.char = "!", 
...
meta_18 <-meta_18[meta_18$condition%in%c("control","STZ"),]
counts_18 <- counts_18[,meta_18$sample]  
rownames(meta_18)<-meta_18$sample  

meta_18$condition <- factor(meta_18$condition, levels = c("control","STZ"))  
...
counts_annot<- cbind(annot_map,counts_18)


# save the counts with gene symbols

write.csv(counts_annot, "GSE179718_raw_counts_symbo.csv", row.names = FALSE)
...
counts_annotated <- counts_18[annot_clean$probe_id,]
...
write.csv(counts_save,"GSE179718_filtered_count_symbols.csv", row.names = FALSE)

# Run limma on cleaned data 

design<- model.matrix(~0 + meta_18$condition)
colnames(design) <- levels(meta_18$condition)
...
write.csv(res_df,"GSE179718_DEGs_1.csv")


res_df$Gene_Symbol <- rownames(res_df)


write.csv(res_df,"GSE179718_DEGs_limma_filtered_main.csv", row.names = FALSE)
...
volc <- EnhancedVolcano(
  ...
  # Titles & Labels
  title = 'GSE179718: STZ vs Control',
...
ggsave("GSE179718_volcano_plot.png", plot = volc, height=8, width= 15,dpi = 1000)
...
annotation_col <- data.frame(
  condition= meta_18$condition,
  row.names = meta_18$sample
)
...
heat <- pheatmap(
  ...
  main = "Top 50 Differentially Expressed Genes of (Z-score) for GSE179718 Dataset"
)

ggsave("GSE179718_heatmap.png", plot = heat, height=10, width= 15,dpi = 1000)
